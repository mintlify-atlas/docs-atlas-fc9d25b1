...
description: "A technical deep-dive into Calculator App's module layout, evaluation pipeline, REPL dispatch loop, and each module's public API."
---
...
```text
calc_app/
├── __init__.py      # re-exports: main, calc, solve_equation,
│                    #             parse_natural_language, parse_fractions
...
The public surface exposed by `__init__.py` is intentionally small:

```python
from .core import main
```

`core.py` in turn imports `calc`, `solve_equation`, `handle_meme`, and `variables` from their respective sibling modules. This means importers of `calc_app` get `main`, while `calc_tests.py` reaches individual functions directly:
